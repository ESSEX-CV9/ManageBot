// src/core/events/clientReady.ts
//
// 启动时向 GUILD_IDS 列表逐个同步斜杠命令（模块无关）。
...
function parseRequiredGuildIdsFromEnv(): string[] {
...
export async function clientReadyHandler(client: Client<true>): Promise<void> {
    ...
    if (!clientId) {
        throw new Error('缺少环境变量 CLIENT_ID。请在 .env 中配置应用的 Application ID。');
    }

    const token = normalizeDiscordToken(process.env.DISCORD_TOKEN);
    const rest = new REST({ version: '10' }).setToken(token);
    // 同样把代理交给这个独立的 REST 实例（命令注册用），否则在被墙网络下会直连超时。
    if (proxyAgent) rest.setAgent(proxyAgent);
    ...
    try {
        ...
        for (const guildId of targetGuildIds) {
            ...
            previewLines.push(guild ? `- ${guild.name} (${guild.id})` : `- [不可访问或机器人未加入] (${guildId})`);
        }
        ...
        for (const guildId of targetGuildIds) {
            try {
                ...
                failCount++;
                const guildName = client.guilds.cache.get(guildId)?.name || guildId;
                const msg = guildError instanceof Error ? guildError.message : String(guildError);
                console.error(`❌ 命令注册失败: ${guildName} (${guildId})`, msg);
            }
        }
